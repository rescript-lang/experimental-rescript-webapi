...
add the package to `rescript.json` and open the global module:
...
With `WebAPI.Global` opened, browser globals are available directly. Interface-specific
methods live on public modules such as `WebAPI.Location`, `WebAPI.Document`,
...
Use `WebAPI.FetchGlobal.fetch` for string URLs and `WebAPI.FetchGlobal.fetchWithRequest`
when you already have a `WebAPI.Request.t`.

```ReScript
let response = await WebAPI.FetchGlobal.fetch("https://example.com/api/users")
...
```ReScript
let response = await WebAPI.FetchGlobal.fetch(
...
let response = await WebAPI.FetchGlobal.fetchWithRequest(req)
...
let authenticatedResponse = await WebAPI.FetchGlobal.fetchWithRequest(authenticatedReq)
```
...
DOM globals come from `WebAPI.Global`; DOM values are operated on through public interface
modules.
