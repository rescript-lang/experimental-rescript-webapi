...
test("root rescript.json keeps generated type modules internal", () => {
  ...
  assert.ok(
    !baseSource.public.includes("BaseCSSFontLoading"),
    "src/Base should keep BaseCSSFontLoading internal",
  );
  assert.ok(
    !baseSource.public.includes("BaseEncryptedMediaExtensions"),
    "src/Base should keep BaseEncryptedMediaExtensions internal",
  );
});

test("root rescript.json exposes event aliases through EventTypes without DOM/Event coupling", () => {
  const config = JSON.parse(fs.readFileSync("rescript.json", "utf8"));

  const publicModulesForFeature = (feature) => {
    const source = config.sources.find((source) => source.feature === feature);
    assert.ok(source, `${feature} source entry should exist`);
    assert.ok(Array.isArray(source.public), `${feature} should explicitly list public modules`);
    return source.public;
  };

  const featureDependencies = (feature) => config.features?.[feature] ?? [];

  assert.deepEqual(publicModulesForFeature("WebAPI.EventTypes"), ["EventType"]);

  for (const moduleName of ["EventTypes", "Base__Event", "Base__EventTarget"]) {
    assert.ok(
      !publicModulesForFeature("WebAPI.EventTypes").includes(moduleName),
      `WebAPI.EventTypes should keep ${moduleName} internal`,
    );
  }

  assert.ok(featureDependencies("WebAPI.DOM").includes("WebAPI.EventTypes"));
  assert.ok(featureDependencies("WebAPI.Event").includes("WebAPI.EventTypes"));

  assert.ok(!featureDependencies("WebAPI.DOM").includes("WebAPI.Event"));
  assert.ok(!featureDependencies("WebAPI.Event").includes("WebAPI.DOM"));

  for (const filePath of fs.readdirSync("src/DOM")) {
    if (!filePath.endsWith(".res")) continue;

    const source = fs.readFileSync(`src/DOM/${filePath}`, "utf8");
    assert.doesNotMatch(source, /\bEvent(?:\.|Target\.)/, `src/DOM/${filePath} should not reference WebAPI.Event modules`);
  }
});
