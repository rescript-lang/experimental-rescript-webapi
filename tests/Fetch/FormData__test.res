...
external myForm: DOM.htmlFormElement = "myForm"

let formData = FormData.make(~form=myForm)

// Get a form field - returns formDataEntryValue which could be string or File
let phoneEntry: null<Fetch.formDataEntryValue> = formData->FormData.get("phone")
...
let allImages: array<Fetch.formDataEntryValue> = formData->FormData.getAll("images")
...
let entries: Iterator.t<(string, Fetch.formDataEntryValue)> = formData->FormData.entries
